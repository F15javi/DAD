...
CREATE TABLE `fly_Schedule` (
  `id_FlySchedule` int NOT NULL AUTO_INCREMENT,
  `plate` varchar(7) NOT NULL,
  `time_dep` time NOT NULL,
  `time_arr` time NOT NULL,
  PRIMARY KEY (`id_FlySchedule`),
  UNIQUE KEY `plate_UNIQUE` (`plate`),
  CONSTRAINT ` id_AirportDest` FOREIGN KEY (`id_FlySchedule`) REFERENCES `airport` (`id_airport`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `id_AirportOrig` FOREIGN KEY (`id_FlySchedule`) REFERENCES `airport` (`id_airport`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `id_Gps` FOREIGN KEY (`id_FlySchedule`) REFERENCES `gps` (`id_Gps`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2022-04-06 17:07:18
